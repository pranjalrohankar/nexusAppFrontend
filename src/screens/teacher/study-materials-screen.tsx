...
export default function StudyMaterialsScreen({ onClose }: StudyMaterialsScreenProps) {
  ...
  const [materials, setMaterials] = useState([
    {
      id: 1,
      title: 'Python for Data Science - Intro',
      description: 'Getting started with Python, NumPy and Pandas.',
      type: 'PDF',
    ...
    },
    {
      ...
      description: 'Supervised learning algorithms and decision trees.',
      type: 'PPT',
      course: 'Data Science & ML',
      downloads: 189,
    },
  ...
  const getFilters = () => {
    ...
    const dataCount = materials.filter(m => m.course.toLowerCase().includes('data')).length;
    const fullCount = materials.filter(m => ['PDF', 'PPT'].includes(m.type)).length;
    return [`All (${allCount})`, `Data (${dataCount})`, `Full (${fullCount})`, `UI/UX (0)`];
  };
  ...
  const handleUploadMaterial = () => {
    ...
    setMaterials([newMaterial, ...materials]); // ← New upload appears at top
    Alert.alert('Success', `Uploaded: ${title}`);
  ...
  const handleDelete = (id: number) => {
    Alert.alert('Delete?', 'Are you sure?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete',
        style: 'destructive',
        onPress: () => setMaterials(prev => prev.filter(item => item.id !== id)),
      },
    ]);
  };
    ...
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <View>
          ...
          <Text style={styles.headerSubtitle}>{materials.length} files • 603 total downloads</Text>
        </View>
            ...
            filteredMaterials.map((item) => (
              <View key={item.id} style={styles.materialCard}>
                <View style={styles.materialIcon}>
                  <Ionicons name={item.type === 'PDF' ? 'document-text' : 'reader'} size={32} color="#7B2CBF" />
                </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TouchableOpacity style={styles.dropZone} onPress={handleFileSelect}>
              ...
              <Text style={styles.dropText}>Drop file here or tap to browse</Text>
              <Text style={styles.supportedTypes}>PDF, PPT, DOC, Video, Image, ZIP</Text>
              {selectedFile && <Text style={styles.selectedFileText}>Selected: {selectedFile.name}</Text>}
            </TouchableOpacity>
            ...
            <View style={styles.row}>
              <View style={styles.half}>
                ...
                <TextInput style={styles.input} placeholder="Select Course" value={course} onChangeText={setCourse} />
              </View>
              <View style={styles.half}>
                <Text style={styles.label}>Batch <Text style={styles.required}>*</Text></Text>
                <TextInput style={styles.input} placeholder="Select Batch" value={batch} onChangeText={setBatch} />
              </View>
            </View>
            ...
            <View style={styles.fileTypeContainer}>
              {fileTypes.map((type) => (
                <TouchableOpacity
                  key={type}
                  style={[styles.fileTypeChip, fileType === type && styles.fileTypeChipActive]}
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 13, color: '#E9D5FF' },
  addButton: {
  ...
  filterChipActive: { backgroundColor: '#7B2CBF' },
  filterText: { color: '#64748B', fontWeight: '600' },
  filterTextActive: { color: '#FFFFFF' },

  materialsList: { paddingHorizontal: 20 },
  materialCard: {
    backgroundColor: '#FFFFFF',
    borderRadius: 16,
    padding: 16,
  ...
  materialIcon: { marginRight: 16 },
  materialInfo: { flex: 1 },
  materialTitle: { fontSize: 16, fontWeight: '600', color: '#1E2937' },
  materialDesc: { fontSize: 13, color: '#64748B', marginVertical: 4 },
  materialMeta: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  typeBadge: { backgroundColor: '#F3E8FF', paddingHorizontal: 10, paddingVertical: 2, borderRadius: 8 },
  typeText: { color: '#7B2CBF', fontSize: 12, fontWeight: '600' },
  downloads: { fontSize: 12, color: '#94A3B8' },
  deleteBtn: { padding: 8 },
  ...
  uploadNewText: { color: '#7B2CBF', fontSize: 16, fontWeight: '700' },
  ...
  dropText: { fontSize: 16, fontWeight: '600', color: '#1E2937', marginBottom: 4 },
  supportedTypes: { fontSize: 13, color: '#64748B' },
  ...
  fileTypeContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 24 },
  fileTypeChip: {
    paddingHorizontal: 16,
    paddingVertical: 8,
    borderRadius: 20,
  ...
  fileTypeText: { fontSize: 13, fontWeight: '600', color: '#64748B' },
